...
if os.path.exists(master_excel):
    ...
    if not df_master.empty:
        ...
        df_master['創半年高'] = df_master.apply(lambda r: r['currently_latest_price'] >= half_year_highs.get(r['代號'], 0) if 'currently_latest_price' in r else r['目前最新價'] >= half_year_highs.get(r['代號'], 0), axis=1)
        ...
        st.info("☕ **操盤手紀律：** 今日全市場各大策略大會師，皆未發現符合特徵的標的。")
else:
    st.info(f"☕ **大會師通知：** 在 **{selected_date.strftime('%Y-%m-%d')}** 這天，尚未產出大會師總表。請確認 Actions 執行狀況。")